...
class ApkDownloader(object):
    ...
    def start(self, name, isUpdate=False):
        print 'starting', name, isUpdate
        global logger
        log_file_name = '/tmp/downloader_%s%s.log' % ('update.' if isUpdate else '', name)
        settings.logger = logutils.config_logger(filename=log_file_name, console=True)
        logger = settings.logger
        self.name = name
        while True:
            if isUpdate:
                apks = service.get_update_apk_links(self.name, 250)
                #apks = service.get_update_apk_links(self.name, 298)
            else:
    ...
    def download(self, source_link, key_url, size, fileName=None):
        ...
        try:
            ...
            with open(self.file_path, 'wb') as f:
                logger.info('%s  start! size:%s' % (fileName, size))
                shutil.copyfileobj(r, f)
                service.report_apk_status([ApkStatus(source_link, key_url, self.name, 1, vol_id, fileName)])
                logger.info('%s  finished! size:%s' % (fileName, size))
            r.close()
    ...
    def filter_item(self, key_url, vol_id, source_link, fileName):
        ...
        apk_exist = is_apk_exist(item.package_name, item.version_code, item.file_type, item.is_break, key_url)
        if apk_exist or (item.file_type == 'ipa' and item.is_break == 0) \
                or self.name in CRALW_ONLY_SOURCES:
            result = apkfilter.silent_remove(self.file_path)
            if result:
                logger.info('vol_id:%s, download_url:%s deleted!' % (vol_id, key_url))
                service.remove_apk(
