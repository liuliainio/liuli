...
def get_apk(num):
    try:
        cursor = _db.cursor()
        sql = "SELECT unique_apk.id,name,icon_link,icon_path,app_itunes.source,app_itunes.source_link,rating, unique_apk.version,developer,sdk_support,category, app_itunes.screen_support,unique_apk.apk_size,language,publish_date,downloads,description,images,images_path,qr_link,unique_apk.download_link,last_crawl,vol_id,package_name,version_code,sig,min_sdk_version,is_break,platform,file_type,package_hash FROM app_itunes,unique_apk WHERE app_itunes.source_link = unique_apk.source_link and unique_apk.tag2 is null and unique_apk.img_status=11 limit %d" % num
        cursor.execute(sql)
