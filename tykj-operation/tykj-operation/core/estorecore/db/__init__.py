...
def app_post_process(apps):
    if apps is None:
        return None

    is_list = True
    if isinstance(apps, Cursor):
    ...
    for app in apps:
        # version_code: stored as str in MongoDB (MongoDB does not support uint64); convert to long
        if 'version_code' in app:
            app['version_code'] = long(app['version_code'])
