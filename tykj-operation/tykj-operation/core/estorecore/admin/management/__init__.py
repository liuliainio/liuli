...
from estorecore.models.base import EntityModel
from estorecore.models.app import AppReview
from estorecore.models.push import Message
from estorecore.models.promotion import Feedback
...
def create_permissions_respecting_proxy(app, created_models, verbosity, **kwargs):
    ...
    for klass in app_models:
        ...
        if issubclass(klass, EntityModel) or issubclass(klass, AppReview):   # only support sync_to perms for EntityModel subclass and AppReview
            for perm in _get_sync_to_permissions(klass._meta):
                searched_perms.append((ctype, perm))
        if issubclass(klass, AppReview) or issubclass(klass, Feedback) :   # only support sync_from perms for Feedback and AppReview
            for perm in _get_sync_from_permissions(klass._meta):
                searched_perms.append((ctype, perm))
        if issubclass(klass, Message):
            perm = ('common_push_message', 'push message common function')
            searched_perms.append((ctype,perm))
...
def create_permission(model_class):
    ...
    if issubclass(model_class, EntityModel):
