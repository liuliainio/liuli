...
from settings import LEFT_NAV_MODELS

from admin_tools.theming import items
from estorecore.models.constants import BELONGTOS
...
class DefaultLeftNav(LeftNav):
    ...
    def init_with_context(self, context):
        for k in sorted(LEFT_NAV_MODELS.keys(), key=lambda k: LEFT_NAV_MODELS[k]['order']):
            v = LEFT_NAV_MODELS[k]
            app_list = items.AppList(v['title'], models=v['models'], app_label_order=v['app_label_order'])
            self.children += [app_list]
        self.children += [items.ReportingItem(_('Reporting'))]
