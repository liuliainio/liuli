...
from estoreoperation.analysis import reporting_items
...
class ReportingItem(AppList):

    def __init__(self, title=None, **kwargs):
        """
        ``Reportings`` constructor.
        """
        super(ReportingItem, self).__init__(title, **kwargs)

    def init_with_context(self, context):
        user = context['request'].user
        for url, reporting_name, title in reporting_items:
            if not user.has_perm('auth.view_%s' % reporting_name):
                continue
            title = capfirst(title)
            url = reverse("reporting_chart", args=(url,))
            item = LeftNavItem(title=title, url=url)
            item.children.append(LeftNavItem(title=title, url=url))
            self.children.append(item)
