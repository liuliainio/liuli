...
def search_code(phone='', code=''):
    status = True
    sql_find = """SELECT code,phone FROM codephone where code ='%s' and phone='%s' and use_status=0"""%(code,phone)
    ty_cursor.execute(sql_find)
    rlts = ty_cursor.fetchall()
    if rlts:
        for rs in rlts:
            return rs
        return True
...
def ajaxtest(request):
    ...
    if request.method == 'POST':
        ...
        for pe in up:
            phone = str(pe.phone)
            if phone:
                strcode = genaral_code(phone=phone)
                sql_add = """INSERT INTO codephone(phone,code,created_time,use_status) VALUES('%s','%s','%s','%s')"""%(phone,strcode,now,0)
                ty_cursor.execute(sql_add)
                ty_db.commit()
                print strcode
                if not send_telecode(strtele=phone,strcode=strcode):
                    print 'ajaxtest_app_view'
                    ajax_status = 200
                    ajax_msg  = 'sucess'
                else:
                    ajax_status = 400
                ...
                messages.error(request,'该帐号未绑定手机号码')
#
...
#             code = request.POST['id_verify_code']
#             print code
#         if 'username' in request.POST.keys():
#             username = request.POST['username']
#         user_id = User.objects.get(username=username).pk
